...
void TempSensor::init(void){
	...
	while(temperature == DEVICE_DISCONNECTED){
		temperature = sensor->getTemp(sensorAddress);
	...
	sensor->requestTemperatures();
	wait.millis(750);
	temperature = sensor->getTemp(sensorAddress); // read again. First read is not accurate
	connected = true;
...
void TempSensor::update(void){
	...
	fixed7_9 temperature = sensor->getTemp(sensorAddress);
	if(temperature == DEVICE_DISCONNECTED){
		...
		if(connected == false){
			...
			temperature = sensor->getTemp(sensorAddress); // re-read temperature after proper initialization
		}
	}
	temperature = constrain(temperature, ((int) INT_MIN)>>5, ((int) INT_MAX)>>5)<<5; // sensor returns 12 bits with 4 fraction bits. Store with 9 fraction bits
