...
void handle_proto(char * string) {
  ...
  for (uint16_t i = 0; i < (unsigned)strlen(string); i++){
      ...
      if (answer.startsWith(functions_names[i])) {
        ...
        if (answer.substring(header_length, header_length + 1) == "?") {
          ...
          if(AREST_PARAMS_MODE == 0) {
            int16_t eq_position = answer.indexOf('=', header_length); // Replacing 'magic number' 8 for fixed location of '='
            if (eq_position != -1)
...
void urldecode(String &arguments) {
  char a, b;
  int j = 0;
  for(uint16_t i = 0; i < arguments.length(); i++) {
...
void addStringToBuffer(const char * toAdd, bool quotable){
  ...
  for (uint16_t i = 0; i < strlen(toAdd) && index < OUTPUT_BUFFER_SIZE; i++, index++) {
